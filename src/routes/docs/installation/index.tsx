            ...
            <section className="mx-auto flex max-w-4xl flex-col gap-10 px-6 py-20 text-slate-200 sm:py-28">
                ...
                <header className="text-center sm:text-left">
                    <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl">
                        {currentLanguage === 'en' && (
                            <BBCodeRenderer
                                text={
                                    "Installation — [b class='bg-gradient-to-r from-sky-400 to-cyan-300 bg-clip-text text-transparent']zero-setup today[/b]"
                                }
                            />
                        )}
                        {currentLanguage !== 'en' && (
                            <BBCodeRenderer text={t('Installation — zero-setup today')} />
                        )}
                    </h1>
                    <p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed sm:mt-6 sm:text-xl">
                        {currentLanguage === 'en' && (
                            <BBCodeRenderer
                                text={
                                    "Right now [b class='font-semibold text-white'] you don’t need to install anything[/b]."
                                }
                            />
                        )}
                        {currentLanguage !== 'en' && (
                            <BBCodeRenderer
                                text={t('Right now you don’t need to install anything.')}
                            />
                        )}
                        ...
                        {currentLanguage === 'en' && (
                            <BBCodeRenderer
                                text={
                                    "Find a component, copy the snippet, paste it into your [b class='font-semibold text-white']React + Tailwind[/b] project — that’s it."
                                }
                            />
                        )}
                        {currentLanguage !== 'en' && (
                ...
                <ul className="grid gap-6 sm:grid-cols-2">
                    ...
                    <li className="flex items-start gap-3 rounded-lg bg-white/5 p-4 ring-1 ring-white/10">
                        ...
                        <div className="text-sm leading-relaxed">
                            <span className="font-semibold text-white">Drop-in friendly.</span>{' '}
                            Fully typed, theme-aware and easy to extend with your own flavors.
                        </div>
                    </li>
                </ul>
                ...
                <div className="rounded-lg bg-white/5 p-6 ring-1 ring-white/10">
                    ...
                    <ol className="space-y-3 text-sm leading-relaxed sm:text-base">
                        ...
                        <li className="flex items-baseline gap-2">
                            ...
                            {t('Paste it into your project')} — Tailwind & Framer Motion{' '}
                            {t('are probably already there')}.
                        </li>
                        <li className="flex items-baseline gap-2">
                            <span className="rounded bg-sky-500 px-2 py-0.5 text-xs font-semibold text-black">
                                3
                            </span>
                            Adjust classes, motion props, or React logic{' '}
                            {t('to fit your design language')}.
                        </li>
                    </ol>
                </div>
                ...
                <div className="rounded-lg bg-yellow-500/10 p-6 ring-1 ring-yellow-500/30">
                    ...
                    <p className="text-sm leading-relaxed sm:text-base">
                        {currentLanguage === 'en' && (
                            <BBCodeRenderer
                                text={
                                    "We’re working on an [b class='font-semibold text-white']npm package[/b] so you can [code class='inline-block']npm install flowscape[/code] and pick only the pieces you need. Expect"
                                }
                            />
                        )}
                        {currentLanguage !== 'en' && (
                            <BBCodeRenderer
                                text={t(
                                    'We’re working on an npm package so you can npm install flowscape and pick only the pieces you need. Expect',
                                )}
                            />
                        )}{' '}
                        tree-shakable ESM bundles, shadcn-style extraction and CLI{' '}
